...
//   Hard coded orders for Testing purpose
//   const orders = [
...
//   API call when the app mounts using useEffect
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await axios.get('http://127.0.0.1:8000/orders/orders_demo/not_delivered'); // API endpoint to get the deliveries pending
        const transformedOrders = response.data.map((order) => {
          const timeLeft = new Date(order.timeLeft).toDateString; // Convert timeLeft to string object
          return {
            ...order,
            timeLeft: timeLeft, // Ensure timeLeft is not negative
          };
        });
